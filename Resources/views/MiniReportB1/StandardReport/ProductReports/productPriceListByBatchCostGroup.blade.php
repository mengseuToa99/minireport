...
@include("minireportb1::MiniReportB1.components.back_to_dashboard_button")
        ...
        <div id="filter-container" class="filter-container"> 
            <div class="button-group">
                <button id="selectCategoryButton" class="btn" data-toggle="modal" data-target="#categoryModal">
                    ...
                    <button id="selectUnitButton" class="btn" data-toggle="modal" data-target="#unitModal">
                        <i class="fas fa-filter"></i> ជ្រើសរើសឯកតា
                    </button>
    ...
    <div class="report-header">
        ...
        តារាងតម្លៃផលិតផលជាកេះតាមក្រុមថ្លៃនិងមួយឯកតា
        </h2>
    </div>
    ...
    <thead>
        <tr>
            <th class="col-xs">#</th>
            <th class="col-md">ឈ្មោះទំនិញ</th>
            <th class="col-xs">ក្រុមទំនិញ</th>
            <th class="col-xs">ឯកតា</th>
            <th class="col-xs">ចំនួនក្នុងមួយកេស</th>

            @foreach ($group_prices as $group_price)
                <th class="col-group-price col-xs" data-group-price-id="{{ $group_price->id }}">{{ $group_price->name }}</th>
            @endforeach
        </tr>
    </thead>
    <tbody>
        ...
        @forelse($grouped_products as $product)
            @php
                // Extract the multiplier (155) from the unit_conversion_info if it exists
                $multiplier = !empty($product['unit_conversion_info']) 
                    ? (float)explode(' ', explode(' = ', $product['unit_conversion_info'])[1])[0]
                    : $product['unit_base_multiplier'];
                
                $base_price = $product['max_purchase_price'];
            @endphp
            <tr data-category-id="{{ $product['category_id'] }}" data-product-name="{{ $product['product_name'] }}">
                ...
                <td>
                    @if(!empty($product['unit_conversion_info']))
                        @php
                            $parts = explode(' = ', $product['unit_conversion_info']);
                            echo $parts[0]; // Shows just "កេស"
                        @endphp
                    @else
                        {{ $product['unit_actual_name'] }}
                    @endif
                </td>
                <td class="number">
                    @if(!empty($product['unit_conversion_info']))
                        @php
                            $parts = explode(' = ', $product['unit_conversion_info']);
                            echo $parts[1]; // Shows "155 sabuដុំ"
                        @endphp
                    @else
                        {{ number_format($product['unit_base_multiplier'], 0, '.', '') }}
                    @endif
                </td>
                ...
                @foreach ($group_prices as $group_price)
                    <td class="number">
                        @if(isset($product['group_prices'][$group_price->id]))
                            {{ number_format($product['group_prices'][$group_price->id] * $multiplier, 2) }}$
                            ({{ number_format($product['group_prices'][$group_price->id] , 2) }}$) 
        ...
        @empty
            <tr>
                <td colspan="{{ 7 + count($group_prices) }}" class="text-center">No products found</td>
            </tr>
        @endforelse
    </tbody>
</table>
...
<div class="modal fade" id="unitModal" tabindex="-1" role="dialog" aria-labelledby="unitModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="unitModalLabel">Select Units to Display</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span>
                </button>
            </div>
            <div class="modal-body">
                @php
                    // Extract all unique units from the products
                    $units = [];
                    foreach ($grouped_products as $product) {
                        $unit = !empty($product['unit_conversion_info']) 
                            ? explode(' = ', $product['unit_conversion_info'])[0] 
                            : $product['unit_actual_name'];
                ...
                @foreach ($units as $unit)
                    <div class="form-check">
                        <input class="form-check-input unit-checkbox" type="checkbox" value="{{ $unit }}" 
                            id="unit{{ $loop->index }}" checked style="transform: scale(2.5);">
                        <label class="form-check-label" for="unit{{ $loop->index }}" style="margin-left: 8px;">
                            {{ $unit }}
                        </label>
                    </div>
                @endforeach
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                <button type="button" class="btn btn-sm btn-outline-primary" id="selectAllUnits">Select All</button>
                <button type="button" class="btn btn-sm btn-outline-danger" id="unselectAllUnits">Unselect All</button>
                <button type="button" class="btn btn-primary" id="applyUnitFilter">Apply</button>
            </div>
        </div>
    </div>
</div>
...
@section('javascript')
    ...
    <script>
        // Unit Filtering
document.getElementById('selectUnitButton')?.addEventListener('click', function (e) {
    e.preventDefault();
    e.stopPropagation();
...
// Unselect All Units
document.getElementById('unselectAllUnits')?.addEventListener('click', function () {
    document.querySelectorAll('.unit-checkbox').forEach(checkbox => {
        checkbox.checked = false;
    });
});
...
// Apply Unit Filter
document.getElementById('applyUnitFilter')?.addEventListener('click', function () {
    const selectedUnits = Array.from(document.querySelectorAll('.unit-checkbox:checked')).map(
        checkbox => checkbox.value
    );

    document.querySelectorAll('#productgrouppriceall tbody tr').forEach(row => {
        const unitCell = row.querySelector('td:nth-child(4)'); // 4th column is the unit column
        if (unitCell) {
            const unitText = unitCell.textContent.trim();
            const displayUnit = selectedUnits.length === 0 || selectedUnits.some(unit => unitText.includes(unit));
            row.style.display = displayUnit ? '' : 'none';
        }
    });
