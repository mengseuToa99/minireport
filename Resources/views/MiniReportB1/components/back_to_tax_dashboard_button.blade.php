...
<script>
    document.getElementById('goBackButton').addEventListener('click', function() {
        // Navigate to the dashboard with a tax tab parameter
       window.history.back();
    });
</script>

<script>
    // Add this script to the main dashboard page to handle the #tax hash
    if (window.location.hash === '#tax') {
        document.addEventListener('DOMContentLoaded', function() {
            // Use setTimeout to ensure the tabs have been initialized
            setTimeout(function() {
                // Find and click the tax tab to activate it
                const taxTab = document.getElementById('tax-tab');
                if (taxTab) {
                    taxTab.click();
                    console.log('Tax tab activated');
                }
            }, 100);
        });
    }
