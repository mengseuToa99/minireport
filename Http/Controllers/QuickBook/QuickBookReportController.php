...
namespace Modules\MiniReportB1\Http\Controllers\QuickBook;
...
class QuickBookReportController extends Controller
{
    /**
     * All Utils instance.
     */
    ...
    // Controller: IncomeStatementController.php
    ...
    public function IncomeStatement(Request $request, $year = null)
    {
        ...
        return view('minireportb1::MiniReportB1.card-menu.business_overview.income_statement')
            ->with(compact(
                'accounts',
                'account_sub_types',
    ...
    public function FinancialPosition()
    {
        ...
        return view('minireportb1::MiniReportB1.card-menu.business_overview.Statement_of_Financial_Position')
            ->with(compact(
                'accounts',
                'account_sub_types',
                'account_detail_types',
    ...
    public function balanceSheet()
    {
        ...
        return view('minireportb1::MiniReportB1.card-menu.business_overview.balance_sheet')
    ...
    public function balanceSheetCompare()
    {
        ...
        return view('minireportb1::MiniReportB1.card-menu.favourites.balance_sheet_compare')
    ...
    public function getProfitLoss(Request $request)
    {
        ...
        // Default date range (current financial year or custom)
        ...
        // Filter account types to include only 'income' and 'expenses'
        $filtered_account_types = array_filter($account_types, function ($key) {
            return in_array($key, ['income', 'expenses']);
        }, ARRAY_FILTER_USE_KEY);

        // Fetch account sub-types based on the filtered primary types
        $account_sub_types = AccountingAccountType::where('account_type', 'sub_type')
            ->where(function ($q) use ($business_id) {
                $q->whereNull('business_id')
                    ->orWhere('business_id', $business_id);
        ...
        return view('quickbooks::favourites.profit_and_loss', compact('business_locations', 'accounts', 'filtered_account_types', 'account_sub_types', 'start_date', 'end_date'));
    }
    ...
    public function getCustomerSuppliers(Request $request)
    {
        ...
        return view('quickbooks::favourites.quarterly_report') // Ensure the view name is correct
            ->with(compact('customer_group', 'types', 'business_locations', 'contact_dropdown'));
    }


    public function getProfitLossComparison(Request $request)
    {
        ...
        // Determine the date range (current financial year or custom)
        if (!empty($request->start_date) && !empty($request->end_date)) {
            $start_date = $request->start_date;
            $end_date = $request->end_date;
        } else {
        ...
        // Get dropdown of business locations
        $business_locations = BusinessLocation::forDropdown($business_id, true);

        // Get accounting primary types (income and expenses)
        $account_types = AccountingAccountType::accounting_primary_type();
        ...
        // Get account sub-types based on the filtered primary types
        $account_sub_types = AccountingAccountType::where('account_type', 'sub_type')
            ->where(function ($q) use ($business_id) {
                $q->whereNull('business_id')
                    ->orWhere('business_id', $business_id);
        ...
        // Main query to retrieve accounts along with child accounts and balances
        $query = AccountingAccount::where('business_id', $business_id)
            ->whereNull('parent_account_id')
            ->with([
                'child_accounts' => function ($query) use ($balance_formula, $start_date, $end_date, $previous_start_date, $previous_end_date) {
                    $query->select([
                        ...
                        JOIN accounting_accounts AS AA ON AAT.accounting_account_id = AA.id 
                        WHERE AAT.accounting_account_id = accounting_accounts.id 
                        AND AAT.operation_date BETWEEN '$previous_start_date' AND '$previous_end_date') AS previous_balance"),
                        'accounting_accounts.*'
                    ]);
                },
                'child_accounts.detail_type',
            ...
            ->select([
                // Current period balance for main accounts
                DB::raw("(SELECT $balance_formula
                FROM accounting_accounts_transactions AS AAT 
                JOIN accounting_accounts AS AA ON AAT.accounting_account_id = AA.id 
                WHERE AAT.accounting_account_id = accounting_accounts.id 
                ...
                // Previous period balance for main accounts
                DB::raw("(SELECT $balance_formula
                FROM accounting_accounts_transactions AS AAT 
                JOIN accounting_accounts AS AA ON AAT.accounting_account_id = AA.id 
                WHERE AAT.accounting_account_id = accounting_accounts.id 
        ...
        // Get the accounts based on the query
        $accounts = $query->get();

        $total_revenue = $accounts->where('account_type', 'Revenue')->sum('balance');

        // Calculate total expenses
        $total_expenses = $accounts->where('account_type', 'Expense')->sum('balance');

        // Calculate net earnings
        $net_earnings = $total_revenue - $total_expenses;

        // Return the view with the necessary data
        return view('quickbooks::business_overview.profit_and_loss_comparison', compact('business_locations', 'accounts', 'filtered_account_types', 'account_sub_types', 'start_date', 'end_date', 'previous_end_date', 'previous_end_date', 'net_earnings'));
    }

    public function getProfitLossComparisonYtd(Request $request)
    {
        // Check user permissions
        if (!auth()->user()->can('profit_loss_report.view')) {
            abort(403, 'Unauthorized action.');
        ...
        // Calculate the YTD start date (from 1st January of the current year)
        $ytd_start_date = \Carbon\Carbon::parse('first day of January ' . \Carbon\Carbon::parse($end_date)->format('Y'))->format('Y-m-d');
        ...
        $query = AccountingAccount::where('business_id', $business_id)
            ->whereNull('parent_account_id')
            ->with([
                'child_accounts' => function ($query) use ($start_date, $end_date, $ytd_start_date) {
                    $query->select([
                        'accounting_accounts.*',
                        DB::raw("(
                            SELECT SUM(amount)
                            FROM accounting_accounts_transactions AS AAT 
                            WHERE AAT.accounting_account_id = accounting_accounts.id 
                            AND AAT.operation_date BETWEEN '$start_date' AND '$end_date'
                        ) AS balance"),
                        DB::raw("(
                            SELECT SUM(amount)
                            FROM accounting_accounts_transactions AS AAT 
                            WHERE AAT.accounting_account_id = accounting_accounts.id 
                            AND AAT.operation_date BETWEEN '$ytd_start_date' AND '$end_date'
                        ) AS ytd_balance"), // Calculate YTD balance
                    ]);
                },
                'detail_type',
                'account_sub_type',
            ...
            ->select([
                ...
                DB::raw("(
                    ...
                    WHERE AAT.accounting_account_id = accounting_accounts.id 
                    AND AAT.operation_date BETWEEN '$ytd_start_date' AND '$end_date'
                ) AS ytd_balance"), // YTD balance for parent accounts
            ]);

        // Get the accounts based on the query
        $accounts = $query->get();

        // Filter accounts by revenue and expense and sum their YTD balances
        $total_revenue = $accounts->filter(function ($account) {
            return $account->account_sub_type->account_primary_type == 'income';
        })->sum('ytd_balance');

        $total_expenses = $accounts->filter(function ($account) {
            return $account->account_sub_type->account_primary_type == 'expenses';
        })->sum('ytd_balance');
        ...
        // Return the view with the necessary data
        return view('quickbooks::business_overview.profit_and_loss_comparison_ytd', compact('business_locations', 'accounts', 'filtered_account_types', 'account_sub_types', 'start_date', 'end_date', 'net_earnings'));
    }



    public function getQuarterlyProfitLossSummary(Request $request)
    {
        if (!auth()->user()->can('profit_loss_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        // Set start and end date based on request or financial year
        if (!empty($request->start_date) && !empty($request->end_date)) {
            $start_date = $request->start_date;
            $end_date = $request->end_date;
        } else {
        ...
        $quarters = [
            ...
            'Q4' => ['start' => '2024-10-01', 'end' => '2024-10-10'],
        ];

        // Create the query for accounts
        $accounts = AccountingAccount::where('business_id', $business_id)
            ->whereNull('parent_account_id')
            ->with([
                'child_accounts' => function ($query) use ($balance_formula, $quarters) {
                    foreach ($quarters as $quarter => $dates) {
                        ...
                        JOIN accounting_accounts AS AA ON AAT.accounting_account_id = AA.id 
                        WHERE AAT.accounting_account_id = accounting_accounts.id 
                        AND AAT.operation_date BETWEEN '{$dates['start']}' AND '{$dates['end']}') AS balance_$quarter"),
                            'accounting_accounts.*'
                        ]);
                    }
                },
        ...
        if ($request->filled('account_type')) {
            $accounts->where('accounting_accounts.account_primary_type', $request->input('account_type'));
        }
        if ($request->filled('status')) {
            $accounts->where('accounting_accounts.status', $request->input('status'));
        }

        // Execute the query and get the accounts
        $accounts = $accounts->get();

        return view('quickbooks::business_overview.quarterly_profit_and_loss_summary', compact(
            'business_locations',
            'accounts',
    ...
    public function getProfitLossByMonth(Request $request)
    {
        if (!auth()->user()->can('profit_loss_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        $accounts = AccountingAccount::where('business_id', $business_id)
            ->whereNull('parent_account_id')
            ->with([
                'child_accounts' => function ($query) use ($balance_formula, $months) {
                    $query->select(['accounting_accounts.*'])
                        ->with(['transactions' => function ($q) use ($balance_formula, $months) {
                            foreach ($months as $month) {
                                // Calculating balance for each month
                                $q->selectRaw("
                                    (SELECT $balance_formula 
                                     FROM accounting_accounts_transactions AS AAT 
                                     JOIN accounting_accounts AS AA ON AAT.accounting_account_id = AA.id 
                                     WHERE AAT.accounting_account_id = accounting_accounts.id 
                                     AND DATE_FORMAT(AAT.operation_date, '%Y-%m') = ?) AS balance_$month", [$month]);
                            }
                        }]);
                }
            ])
        ...
        // Return the view with the necessary data
        return view('quickbooks::business_overview.profit_and_loss_by_month', compact('business_locations', 'accounts', 'filtered_account_types', 'account_sub_types', 'start_date', 'end_date', 'months'));
    }

    public function getProfitLossPercentage(Request $request)
    {
        if (!auth()->user()->can('profit_loss_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        $accounts = $query->get();
        $results = $this->calculateNetEarnings($accounts);
        $net_earnings = $results['net_earnings'];
        $accounts_percentage = $results['accounts_percentage'];

        return view('quickbooks::business_overview.profit_and_loss_percentage', compact(
            'business_locations',
            'accounts',
            'filtered_account_types',
            'account_sub_types',
    ...
    public function calculateNetEarnings($accounts)
    {
        // Initialize income and expense totals
        $total_income = 0;
        $total_expenses = 0;

        // Loop through accounts to sum income and expenses
        foreach ($accounts as $account) {
            if ($account->account_primary_type === 'income') {
                $total_income += $account->balance;
            } elseif ($account->account_primary_type === 'expenses') {
                $total_expenses += abs($account->balance); // Use abs to ensure expenses are positive
            }
        }
        ...
        // Calculate percentage of each account relative to total income
        $accounts_percentage = [];
        $total_expenses_percentage = 0;

        // Ensure total_income is positive to avoid division by zero
        if ($total_income > 0) {
            foreach ($accounts as $account) {
                $percentage = 0;
                if ($account->account_primary_type === 'income') {
                    $percentage = ($account->balance / $total_income) * 100;
                } elseif ($account->account_primary_type === 'expenses') {
                    // Expenses are calculated as a percentage of total income
                    $percentage = (abs($account->balance) / $total_income) * 100;
                    $total_expenses_percentage += $percentage;
                }
                $accounts_percentage[$account->id] = $percentage;

                // Debugging output
                \Log::info("Account ID: {$account->id}, Balance: {$account->balance}, Percentage: {$percentage}");
            }
        } else {
            // If total income is zero or negative, set all percentages to zero
            foreach ($accounts as $account) {
                $accounts_percentage[$account->id] = 0;

                // Debugging output
                \Log::info("Account ID: {$account->id}, Balance: {$account->balance}, Percentage: 0%");
            }
        }
        ...
        return [
            ...
            'total_expenses_percentage' => $total_expenses_percentage,
        ];
    }

    public function getProfitLossDetail(Request $request)
    {
        if (!auth()->user()->can('profit_loss_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        // Query to fetch the accounts and their balances
        $query = AccountingAccount::where('business_id', $business_id)
            ->whereNull('parent_account_id')
            ->with([
                'child_accounts' => function ($query) use ($balance_formula, $start_date, $end_date) {
        ...
        return view('quickbooks::business_overview.profit_and_loss_detail', compact(
            'business_locations',
            'accounts',
            'filtered_account_types',
            'account_sub_types',
    ...
    {
        if (!auth()->user()->can('quickbooks.Purchase&Sale')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        return view('quickbooks::Purchase_sell.index')
     ...
     * @return \Illuminate\Http\Response
     */
    public function exportCustomerSuppliers(Request $request)
    {
        $business_id = $request->session()->get('user.business_id');
        $date_range = $request->input('date_range');
        $contact_type = $request->input('contact_type');
        $cs_report_location_id = $request->input('cs_report_location_id');

        return Excel::download(new CustomerSupplierExport($business_id, $date_range, $contact_type, $cs_report_location_id), 'customer-suppliers-report.xlsx');
    }
    ...
    //     return view('quickbooks::Supplier_customer.index')
    //         ->with(compact('customer_group', 'types', 'business_locations', 'contact_dropdown'));
    // }
    ...
    {
        if (!auth()->user()->can('quickbooks.Stock_Report')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        if ($request->ajax()) {
            ...
            if (!empty(request()->start_date) && !empty(request()->end_date)) {
                ...
                $products->whereDate('p.created_at', '>=', $start)
        ...
        return view('quickbooks::Stock_report.index')
    ...
    {
        if (!auth()->user()->can('quickbooks.Tax_Report')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        return view('quickbooks::Tax_report.index')
    ...
    {
        if (!auth()->user()->can('Trending_Products')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        return view('quickbooks::Trending_product.index')
    ...
    public function getExpenseReport(Request $request)
    {
        ...
        return view('quickbooks::expense.index')
    ...
    {
        if (!auth()->user()->can('quickbooks.Stock_Adjustment_Report')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        return view('quickbooks::Stock_adjustment.stock_adjustment_report')
            ->with(compact('business_locations'));
    }
    ...
    {
        if (!auth()->user()->can('quickbooks.Register_Report')) {
            abort(403, 'Unauthorized action.');
        }
        $business_id = $request->session()->get('user.business_id');
        ...
        return view('quickbooks::Register_report.index')
    ...
    {
        if (!auth()->user()->can('quickbooks.Sales_Representative_Report')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        return view('quickbooks::Sales_representative.index')
    ...
    public function getSalesRepresentativeTotalExpense(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return sales_representative_report;
        }
    }
    ...
    {
        if (!auth()->user()->can('Stock_Expiry_Report')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        return view('quickbooks::Stock_expire.stock_expiry_report')
    ...
    {
        if (!auth()->user()->can('Customer_Groups_Report')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        return view('quickbooks::customer_group.index')
    ...
    public function getproductPurchaseReport(Request $request)
    {
        ...
        return view('quickbooks::Product_purchase.index')
    ...
    public function getproductSellReport(Request $request)
    {
        ...
        return view('quickbooks::Product_sell.index')
            ->with(
                compact(
                    'business_locations',
                    'customers',
    ...
    public function purchasePaymentReport(Request $request)
    {
        ...
        return view('quickbooks::Purchase_payment.index')
    ...
    {
        if (!auth()->user()->can('quickbooks.Sell_Payment_Report')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        return view('quickbooks::Sell_payment_report.index')
    ...
    public function getServiceStaffReport(Request $request)
    {
        ...
        return view('service_staff_report')
    ...
    public function itemsReport()
    {
        ...
        if (!auth()->user()->can('quickbooks.Items_Report')) {
            abort(403, 'Unauthorized action.');
        }
        if (request()->ajax()) {
        ...
        return view('quickbooks::Items_report.index')->with(compact('suppliers', 'customers', 'business_locations'));
    }
    ...
    public function getStockValue()
    {
        ...
        //Get Closing stock
        $closing_stock_by_pp = $this->getOpeningClosingStock(
            $business_id,
            $started_date,
            $ended_date,
            $end_date,
    ...
    public function getOpeningClosingStock($business_id, $started_date, $ended_date, $date, $location_id, $is_opening = false, $by_sale_price = false, $filters = [])
    {
        $query = PurchaseLine::join(
            'transactions as purchase',
            'purchase_lines.transaction_id',
        ...
        if (!empty($started_date) && !empty($ended_date)) {
            $start = $started_date;
            $end = $ended_date;
    ...
    {
        $business_id = request()->session()->get('user.business_id');
        if (!auth()->user()->can('quickbooks.Activity_Log')) {
            abort(403, 'Unauthorized action.');
        }
        $transaction_types = [
        ...
        return view('quickbooks::business_overview.activity_log')->with(compact('users', 'transaction_types'));
    }
    ...
    {
        return view('quickbooks::AccountingReport.index');
    }
}
